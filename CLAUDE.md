...
- **Alta de cliente nuevo**: sigue creando una fila nueva e independiente en `clientes` cada vez (no busca coincidencias con otros paneles) — si el mismo negocio se da de alta después en otro panel, hay que reconciliarlo a mano (renombrar uno de los dos para que casen, y opcionalmente fusionar histórico).
