...
- **El Excel maestro (`grupos_millenium_MAESTRO.xlsx`) es la fuente de verdad de los derechos de cada cliente** — según indicación explícita del usuario, tiene preferencia sobre cualquier dato de facturación: si un cliente está en el maestro sin derecho a una categoría, no debe puntuar en ella aunque el ERP traiga consumo real de esa categoría (así ya funciona `computeClientCat`); un cliente que **no** aparece en el maestro sigue puntuando en todo por defecto (`puntos_mecanica`/`puntos_carroceria` = `true` de serie) hasta que se clasifique. **Autoactualización desde la app**: "Importar Consumo" → tipo de fichero **"Clasificación Millennium (Mecánica/Carrocería)"** (`procesarFicheroMillennium()`/`confirmImportMillennium()`) sube el Excel maestro directamente (columnas CÓDIGO/CLIENTE/MECÁNICA/CHAPA, valores SI/NO) y actualiza `puntos_mecanica`/`puntos_carroceria` por `codigo_erp` normalizado — solo toca clientes que ya existen en la app; los códigos del maestro sin cliente todavía (sin consumo previo) se listan en la revisión pero se ignoran sin acción, consistente con el resto del sistema. Con esto, corregir la clasificación de un cliente ya no requiere pasar por SQL/migración manual — solo actualizar el Excel maestro y volver a importarlo.
